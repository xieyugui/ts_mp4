    ...
    ngx_http_mp4_trak_t *trak)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "mp4_update_stts_atom atom_size:%luD", atom_size);
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "---stts start----- last trak->time_to_sample_entries:%ui", trak->time_to_sample_entries);
    ...
    ngx_http_mp4_trak_t *trak, ngx_uint_t start)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "mp4 stts crop trak->time_to_sample_entries:%ui", entries);
    ...
    ngx_http_mp4_trak_t *trak)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "start sync sample entries:%uD", trak->sync_samples_entries);

    ngx_http_mp4_crop_stss_data(mp4, trak, 1);
    ngx_http_mp4_crop_stss_data(mp4, trak, 0);

    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "end sync sample entries:%uD", trak->sync_samples_entries);
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "ngx_http_mp4_update_stss_atom atom_size:%luD", atom_size);
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "----stss start-----sync_samples_entries:%u",trak->sync_samples_entries);
    ...
    ngx_http_mp4_trak_t *trak)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "start composition offset entries:%uD",
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "end composition offset entries:%uD",
                   trak->composition_offset_entries);
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "ngx_http_mp4_update_ctts_atom atom_size:%luD", atom_size);
    trak->size += atom_size;
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "----ctts start-----composition_offset_entries:%u",trak->composition_offset_entries);
    ...
    while (entry < end) {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                       "count:%uD, offset:%uD",
                       ngx_mp4_get_32value(entry->count), ngx_mp4_get_32value(entry->offset));
        entry++;
    }
...
found:
    ...
    if (start) {
        ...
        trak->composition_offset_entries = entries;
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                       "ngx_http_mp4_crop_ctts_data count:%uD, composition_offset_entries:%uD",
                       count - rest, trak->composition_offset_entries);
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                       "ngx_http_mp4_crop_ctts_data count:%uD, composition_offset_entries:%uD",
                       rest, trak->composition_offset_entries);
    }
}
    ...
    ngx_http_mp4_trak_t *trak)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "start sample-to-chunk entries:%uD",
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "end sample-to-chunk entries:%uD",
                   trak->sample_to_chunk_entries);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "atom_size:%luD, sample_to_chunk_entries:%u", atom_size, trak->sample_to_chunk_entries);
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "---stsc start---sample_to_chunk_entries:%uD", trak->sample_to_chunk_entries);
    ...
    while (entry < end) {
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                       "chunk:%uD, "
                               "samples:%uD, id:%uD", chunk, samples, id);
    ...
    ngx_http_mp4_trak_t *trak, ngx_uint_t start)
{
    ...
    if (start) {
        ...
        if (data) {
            ...
            entries--;
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                           "sample:%uD, entries:%uD", samples, entries);
...
found:
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "entries:%uD, prev_samples:%uD", entries, prev_samples);
    target_chunk = chunk - 1;
    target_chunk += start_sample / samples;
    chunk_samples = start_sample % samples;

    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "target_chunk:%ui, chunk_samples:%ui",
    ...
    ngx_http_mp4_trak_t *trak)
{
    ...
    if (data) {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                       "=---stsz-start---entries:%uL", entries);
        uint32_t             *entry, sample;
        entry = (uint32_t *) data->pos;
        end = (uint32_t *) data->last;
        ...
        while (entry < end) {
            ...
            ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "stsz:%uD", sample);
    ...
    ngx_http_mp4_trak_t *trak)
{
    ...
    if (mp4->length) {
        ...
        data->last = data->pos + entries * sizeof(uint32_t);
        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                       " if (mp4->length) trak->end_chunk:%u,trak->start_chunk:%u,entries=%llu", trak->end_chunk,trak->start_chunk, entries);
        if (entries) {
            trak->end_offset =
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                       "else trak->end_chunk:%u,trak->start_chunk:%u,entries=%llu", trak->end_chunk,trak->start_chunk, entries);
    }
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "atom_size:%uL, entries:%u", atom_size, entries);
    trak->size += atom_size;
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "=---stco-start---entries:%uL", entries);
    uint32_t        *end,     *entry, sample;
    entry = (uint32_t *) data->pos;
    end = (uint32_t *) data->last;
    ...
    while (entry < end) {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "stco:%uD", sample);
